...
\begin{description}
 \item[Turboveg] is a desktop program, written in VisualBasic. It provides basic functions to enter, import, maintain and export vegetation data. From the 2 000 000 vegetation plots registered in \url{http://www.GIVD.info} approximately 1.5 million are stored in Turboveg databases format.
