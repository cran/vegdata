...
In GermanSL versions 1.1 to 1.3 additional information for every taxon is stored in an extra file (tax.dbf) which could be used with option \textit{detailed = TRUE}. 
