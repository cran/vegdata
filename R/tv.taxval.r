...
subdiv = c('conflict', 'adapt', 'preserve'), ag = c('conflict', 'adapt','preserve'), mono = c('lower','higher','all','preserve'), monolist = "monotypic-D", uncertain = NULL, genus = c('preserve','delete'), quiet = FALSE, sysPath = FALSE, ...) 
{
...
if (mono %in% c("lower", "higher", "all")) {
  ...
  ## Determined only to Aggregate level
    obsspec <- unique(obs$SPECIES_NR)
...
### Extent of taxon interpretation
  sl <- species[grep("\ s.\ l.$", species$ABBREVIAT, perl=TRUE), c(1:5, 11, 13, 14, 15)]
  sl$to_check <- sub("\ s.\ l.$", "", sl$ABBREVIAT, perl=TRUE)
  sstr <- species[grep("\ s.\ str.$", species$ABBREVIAT, perl=TRUE), c(1:5, 11, 13, 14, 15)]
