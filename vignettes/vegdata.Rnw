...
I confess that it is a strange and complete artificial example. Starting with 20 names in the beginning only 11 names survived the valuation. All others had to be converted to give only information about consistent taxon concepts.
...
@
will use the first name ('QUERR-R') for the replacement column with the sum of the selected columns.
...
If you have to combine different taxonomic reference lists, function \texttt{tv.compRefl} might be a starting point, comparing species numbers and/or species names of both lists.
<<eval=FALSE>>=
