...
obs.tax$Name <-  species$taxonName[match(obs.tax$SPECIES_NR, species$SPECIES_NR)]
head(obs.tax[,1:4])
@
...
<<Taxon>>=
obs.taxval$Taxon <-  species$taxonName[match(obs.taxval$SPECIES_NR, species$SPECIES_NR)]
...
tmp$Taxon <-  species$taxonName[match(tmp$SPECIES_NR, species$SPECIES_NR)]
@
<<print.coarsen>>=
...
\section[label=ESVeg]{ESVeg, a XML exchange format for vegetation data}
